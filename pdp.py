...
```
embed = discord.Embed(
    title="📌 Import d'images Pinterest",
    description="**Utilisez le bookmarklet pour extraire automatiquement les URLs !**",
    color=0xe74c3c
)
embed.add_field(
    name="🔗 Étape 1 : Installer le bookmarklet (une seule fois)",
    value=(
        ...
        "3. Une fois installé, passez à l'étape 2"
    ),
    inline=False
)
embed.add_field(
    name="📸 Étape 2 : Extraire les URLs d'un board Pinterest",
    value=(
        ...
        "4. Les URLs sont copiées dans votre presse-papier"
    ),
    inline=False
)
embed.add_field(
    name="📋 Étape 3 : Importer dans Discord",
    value=(
...
await ctx.send("**📎 Collez vos URLs d'images (extraites via le bookmarklet ou manuellement) :**")
try:
    ...
    for url in all_urls:
        ...
        if '/60x60/' in url or '/75x75/' in url or '/50x50/' in url:
            logging.info(f"Miniature ignorée: {url}")
            continue
    ...
    if not image_urls:
        return await ctx.send(
            ...
            "Ou faites clic droit sur une image Pinterest → **Copier l'adresse de l'image**\n"
...
# ---- INSERTION EN DB (VERSION CORRIGÉE) ----
...
for img_url in selected_urls:
    try:
        # ✅ FIX: Utiliser RETURNING pour vérifier l'insertion
        cur.execute(
        ...
        if result:  # ✅ Insertion réussie
            ...
            logging.info(f"✅ Inséré: {img_url[:50]}...")
        ...
        duplicates += 1
        conn.rollback()
        logging.info(f"⚠️ Doublon: {img_url[:50]}...")
...
if errors > 0:
    final_embed.add_field(name="❌ Erreurs", value=str(errors), inline=True)
...
if webhook_sent:
    ...
    final_embed.add_field(name="📤 Webhook", value="⚠️ Non configuré", inline=False)
...
# COMMANDE !DBRESET (ADMIN)

# –––––––––––

@bot.command(name=“dbreset”)
async def dbreset_cmd(ctx):
“”“Réinitialiser complètement la table images”””

```
# ⚠️ SÉCURITÉ : Limiter aux admins si nécessaire
# if ctx.author.id != YOUR_DISCORD_ID:
#     return await ctx.send("❌ Commande réservée aux admins")

await ctx.send("⚠️ **ATTENTION : Ceci va supprimer TOUTES les images de la DB !**\nTapez `CONFIRM` pour continuer.")

def check(m):
    return m.author == ctx.author and m.channel == ctx.channel

try:
    confirm_msg = await bot.wait_for("message", timeout=30, check=check)
    if confirm_msg.content.strip().upper() != "CONFIRM":
        return await ctx.send("❌ Opération annulée.")
except asyncio.TimeoutError:
    return await ctx.send("⏱️ Temps écoulé. Opération annulée.")

status_msg = await ctx.send("🔄 **Réinitialisation en cours...**")
...
try:
    ...
    # Créer l'index
    cur.execute("""
        CREATE INDEX idx_category_used 
        ON images(category, used) WHERE used = FALSE;
    """)
    ...
    await status_msg.edit(content="✅ **Table réinitialisée avec succès !**\n\nVous pouvez maintenant importer des images avec `!url`")
    
except Exception as e:
    logging.error(f"Erreur reset DB: {e}")
    cur.close()
    conn.close()
    await status_msg.edit(content=f"❌ **Erreur lors de la réinitialisation :** {e}")
```
